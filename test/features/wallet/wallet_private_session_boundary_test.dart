...
void main() {
  ...
  group('after lock, no consumer can resolve private material', () {
    setUp(() {
      signingMaterial.loadPrivateCapability(walletId: _walletId, seed: _seed());
    });
  ...
  group('visible catalog', () {
    ...
    test('drops the wallet on lock and emits the new catalog', () async {
      signingMaterial.loadPrivateCapability(walletId: mountedId, seed: _seed());
    ...
    test(
      ...
      () async {
        signingMaterial.loadPrivateCapability(
          walletId: mountedId,
          seed: _seed(),
        );
