...
void main() {
  ...
  test('resolves exactly the requested spendable coins and total', () async {
    ...
    expect(result, isA<Ok<ResolvedSweepInputs, SendFailure>>());
    final sweep = (result as Ok<ResolvedSweepInputs, SendFailure>).value;
    expect(sweep.selectedUtxos, [first, second]);
    expect(sweep.totalSat, 75000);
  });
  ...
  test('fails when a requested coin is missing', () async {
    ...
    expect(result, isA<Err<ResolvedSweepInputs, SendFailure>>());
    expect(
      (result as Err<ResolvedSweepInputs, SendFailure>).failure,
